...
import (
	...
	"github.com/pipe4/lang/pipe4/gogll/parser/bsr"
	"github.com/pipe4/lang/pipe4/gogll/parser/slot"
	"github.com/pipe4/lang/pipe4/gogll/parser/symbols"
	"github.com/pipe4/lang/pipe4/gogll/token"
)
...
func (p *parser) parse() (*bsr.Set, []*Error) {
	...
	for !p.R.empty() {
		...
		switch L {
		case slot.GoGLL0R0: // GoGLL : ∙ImportBlock

			p.call(slot.GoGLL0R1, cU, p.cI)
		case slot.GoGLL0R1: // GoGLL : ImportBlock ∙
		...
		case slot.Import0R0: // Import : ∙ImportName ImportPath

			p.call(slot.Import0R1, cU, p.cI)
		case slot.Import0R1: // Import : ImportName ∙ImportPath
		...
		case slot.Import0R2: // Import : ImportName ImportPath ∙
		...
		case slot.ImportBlock0R0: // ImportBlock : ∙import ( ImportStatements )

			p.bsrSet.Add(slot.ImportBlock0R1, cU, p.cI, p.cI+1)
			p.cI++
			if !p.testSelect(slot.ImportBlock0R1) {
				p.parseError(slot.ImportBlock0R1, p.cI, first[slot.ImportBlock0R1])
				break
			}

			p.bsrSet.Add(slot.ImportBlock0R2, cU, p.cI, p.cI+1)
			p.cI++
			if !p.testSelect(slot.ImportBlock0R2) {
				p.parseError(slot.ImportBlock0R2, p.cI, first[slot.ImportBlock0R2])
				break
			}

			p.call(slot.ImportBlock0R3, cU, p.cI)
		case slot.ImportBlock0R3: // ImportBlock : import ( ImportStatements ∙)

			if !p.testSelect(slot.ImportBlock0R3) {
				p.parseError(slot.ImportBlock0R3, p.cI, first[slot.ImportBlock0R3])
				break
			}

			p.bsrSet.Add(slot.ImportBlock0R4, cU, p.cI, p.cI+1)
			p.cI++
			if p.follow(symbols.NT_ImportBlock) {
				p.rtn(symbols.NT_ImportBlock, cU, p.cI)
			} else {
				p.parseError(slot.ImportBlock0R0, p.cI, followSets[symbols.NT_ImportBlock])
			}
		case slot.ImportName0R0: // ImportName : ∙localDeclarationName

			p.bsrSet.Add(slot.ImportName0R1, cU, p.cI, p.cI+1)
			p.cI++
			if p.follow(symbols.NT_ImportName) {
				p.rtn(symbols.NT_ImportName, cU, p.cI)
			} else {
				p.parseError(slot.ImportName0R0, p.cI, followSets[symbols.NT_ImportName])
			}
		case slot.ImportPath0R0: // ImportPath : ∙string

			p.bsrSet.Add(slot.ImportPath0R1, cU, p.cI, p.cI+1)
			p.cI++
			if p.follow(symbols.NT_ImportPath) {
				p.rtn(symbols.NT_ImportPath, cU, p.cI)
			} else {
				p.parseError(slot.ImportPath0R0, p.cI, followSets[symbols.NT_ImportPath])
			}
		case slot.ImportStatements0R0: // ImportStatements : ∙Import

			p.call(slot.ImportStatements0R1, cU, p.cI)
		case slot.ImportStatements0R1: // ImportStatements : Import ∙

			if p.follow(symbols.NT_ImportStatements) {
				p.rtn(symbols.NT_ImportStatements, cU, p.cI)
			} else {
				p.parseError(slot.ImportStatements0R0, p.cI, followSets[symbols.NT_ImportStatements])
			}
		case slot.ImportStatements1R0: // ImportStatements : ∙Import ImportStatements

			p.call(slot.ImportStatements1R1, cU, p.cI)
		case slot.ImportStatements1R1: // ImportStatements : Import ∙ImportStatements

			if !p.testSelect(slot.ImportStatements1R1) {
				p.parseError(slot.ImportStatements1R1, p.cI, first[slot.ImportStatements1R1])
				break
			}

			p.call(slot.ImportStatements1R2, cU, p.cI)
		case slot.ImportStatements1R2: // ImportStatements : Import ImportStatements ∙

			if p.follow(symbols.NT_ImportStatements) {
				p.rtn(symbols.NT_ImportStatements, cU, p.cI)
			} else {
				p.parseError(slot.ImportStatements1R0, p.cI, followSets[symbols.NT_ImportStatements])
			}
...
var first = []map[token.Type]string{
	...
	{
		token.T_3: "localDeclarationName",
	},
	// Import : ImportName ∙ImportPath
	{
		token.T_4: "string",
